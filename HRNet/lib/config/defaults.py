...
### PRUNING related params
PRUNING = CN()
PRUNING.PRUNE = False
PRUNING.AMOUNT = 0.0  # Pruning ratio

# QUANTIZATION related params
QUANTIZATION = CN()
QUANTIZATION.QUANTIZE = False
QUANTIZATION.QUANTIZE_TYPE = 'int8'  # Quantization Type
QUANTIZATION.QUANTIZE_BITS = 8  # number of bits for quantization

_C.PRUNING = PRUNING
_C.QUANTIZATION = QUANTIZATION
...
def update_config(cfg, args):
    ...
    ### PRUNING & QUANTIZATION initialization
    if 'PRUNING' not in cfg:
        cfg.PRUNING = CN()
        cfg.PRUNING.PRUNE = False
        cfg.PRUNING.AMOUNT = 0.0

    if 'QUANTIZATION' not in cfg:
        cfg.QUANTIZATION = CN()
        cfg.QUANTIZATION.QUANTIZE = False
        cfg.QUANTIZATION.QUANTIZE_TYPE = 'int8'
        cfg.QUANTIZATION.QUANTIZE_BITS = 8
